...
class codonsDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # your code
        # super().__getitem__(index)
        #codon_tensor = codon_to_tensor(self.codons[index], codonsList)  # uncomment for 64 bits OHE
        codon_tensor = codon_to_tensor_12bits(self.codons[index], nucleotideLetters) # uncomment for 12 bits OHE of codons
...
#print('The codon is ', tensor3x4_to_codon(cod.view(3, 1, 4), nucleotideLetters))
...
#print('next codon train=', tensor3x4_to_codon(next_batch_codons[-1].view(3, 1, 4), nucleotideLetters))
...
#print('next codon train=', tensor_to_codon(next_batch_codons[-1], codonsList))
#print('next codon test=', tensor3x4_to_codon(next_batch_codons[-1].view(3, 1, 4), nucleotideLetters))
#print('next aa test=', tensor3_to_sequence(next_batch_aas[-1].unsqueeze(0), aminoAcidLetters))
...
input_features = 12 # 64 features per codon (results from One Hot Encoding of the 64 possible different triplets of 4 nucleotide letters)
output_features = 21
...
class MLP(nn.Sequential): # Multilayer Perceptron Network
    def __init__(self, input_features, output_features, hidden_features_layer1, hidden_features_layer2):
        super().__init__(
            ...
            nn.Linear(hidden_features_layer1, hidden_features_layer2, bias=False),
            #nn.ReLU(), # # could be removed (no activation between the two layers could be possible as well)
        ...
        for x, y in train_codon_loader:
            ...
            if batch_iter in samples_id:
                ...
                for batch_codon in range(x.size()[0]):
                    ...
                    this_triplet = tensor3x4_to_codon(x[batch_codon].view(3, 1, 4), nucleotideLetters)
...
#plt.show()
#save figure:
plt.savefig(r"C:\Users\marcjoiret\Desktop\MARC\TISSUE ENGINEERING and REGENERATIVE MEDICINE\DEEP LEARNING\Project\FIG\MLP12bitOHE1024weights.svg")
plt.savefig(r"C:\Users\marcjoiret\Desktop\MARC\TISSUE ENGINEERING and REGENERATIVE MEDICINE\DEEP LEARNING\Project\FIG\MLP12bitsOHE1024weights.pdf")
...
ThisFileName = "MLP12bitsOHE1024weights.txt"
scoresfile = open(filePATH+ThisFileName, 'w')
headerline = 'MLP64to1024 OHE 12 bits with 40 epochs adjusting for weights.\n'
